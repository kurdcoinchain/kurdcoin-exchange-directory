...
anchors.forEach((anchor) => {
    ...
    anchor.assets.forEach((asset) => {
        ...
        directory.addPair({
            baseBuying: isCounterAsset ? ['KRD', 'native'] : [asset.code, anchor.domain],
            counterSelling: isCounterAsset ? [asset.code, anchor.domain] : ['KRD', 'native'],
        });
    });
});
...
pairIds.forEach((pairId) => {
    ...
    if (pair.baseBuying.code === 'KRD' && pair.baseBuying.issuer === null) {
        delete remainingAssets[`${pair.counterSelling.code}-${pair.counterSelling.issuer}`];
    } else if (pair.counterSelling.code === 'KRD' && pair.counterSelling.issuer === null) {
